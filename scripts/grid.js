...
export class Row {
    ...
    */
    forEachCellBetween(X1, X2, callback, returnVariable) {
...
export class Grid {
    constructor(width = 1, height = 1, opts) {
        ...
        let generateFrom = (opts === null || opts === void 0 ? void 0 : opts.generateFrom) ? opts.generateFrom : undefined;
    ...
    */
    forEachCellInRowBetween(YCoordinate, X1, X2, callback, returnVariable) {
    ...
    */
    forEachCellInColumnBetween(XCoordinate, Y1, Y2, callback, returnVariable) {
        var _a;
        let bottom = Math.min(Y1, Y2);
        let top = Math.max(Y1, Y2);
        for (let y = bottom; y <= top; y++) {
            let cell = (_a = this.row(y)) === null || _a === void 0 ? void 0 : _a.column(XCoordinate);
            if (cell != undefined) {
                if (returnVariable != undefined) {
                    returnVariable = callback(cell, this, returnVariable);
                }
    ...
    */
    forEachCellinArea([X1, Y1], [X2, Y2], callback, returnVariable) {
        let bottom = Math.min(Y1, Y2);
        let top = Math.max(Y1, Y2);
        for (let y = bottom; y <= top; y++) {
            if (returnVariable != undefined) {
    ...
    */
     ...
     * @see Row.fillColumns
     */
    fillRows(width, YAxis, fillCells = true, generateFromX = undefined) {
    ...
    */
    generateOffsetCoordinates(original, other, iterationCounter, referenceIndex, to) {
     ...
     * and relativeY = q
    */
    generateRelativeXYCoordinates(iterationCounter, other) {
